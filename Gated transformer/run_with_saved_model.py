...
path = f'C:\\Users\\Administrator\\Desktop\\cx\\GTN-master\\Gated Transformer 论文IJCAI版\\MTS_dataset\\{file_name}\\{file_name}.mat'  # 拼装数据集路径
...
# 加载保存的模型
net = torch.load(save_model_path, map_location=torch.device('cuda:0'))  # map_location 设置使用的设备
    ...
    for x, y in test_dataloader:
        ...
        if heatMap_or_not:
            for index, sample in enumerate(test_dataset.max_length_sample_inTest):
                if sample.cpu().detach().numpy().tolist() in x.cpu().detach().numpy().tolist():    # 数据转换为numpy格式，转换为data.cpu()
